...
def training(training_type):
    skip = '2'
    if shop1['2']['name'] in player['inventory']:
...
def shop():
    ...
    for key, value in shop1.items():
        print(f'{key} - {value["name"]}: {value["price"]}')

    item = input()
    if item in player['inventory']:
        print(f'У тебя уже есть {shop1[item]["name"]}')
    elif player['money'] >= shop1[item]['price']:
        print(f'Ты успешно приобрёл {shop1[item]["name"]}')
        player['inventory'].append(shop1[item]["name"])
        player['money'] -= shop1[item]['price']
    else:
